...
field in the library header plus the filter fields beside it — `RecipeFilterFields`, rendered
by `RecipeFilterRail` (permanent from `lg`) and `RecipeFilterSheet` (the phone "Filter"
button). Do not re-add a search screen — add a field to `RecipeFilterFields` instead, and it
appears in both surfaces at once. `RecipeFilters.tsx` and its collapsible "Erweiterte Suche"
panel are gone (T8.2); the rule they carried is not.
...
- **A `sticky bottom-*` bar can only be pushed UP, never DOWN**, so on a SHORT page it floats
  ...
  parent height and a flex-GROWN item is not one, so Chromium left the root at its content height
  (493px inside a 695px main) and the bar stayed where it was. The bar also carries `-mb-4` to
  swallow the 1rem of breathing room inside `pb-tabbar`, or a strip of page background shows under
  it.
...
- **THREE SCREENS HAVE NO ARTBOARD, AND THEY WERE DERIVED, NOT INVENTED — do not "correct" them
  ...
  **Two consequences.** An empty screen is the *default* on a fresh install, not an edge case: R41
  ...
  deliberate rather than a missing state. And **`--fg-faint` is `#8d7c67` (dark) / `#7f6f5b`
